...
const vcard = 'BEGIN:VCARD\n' 
            ...
            + 'FN:Affis Admin\n' 
            + 'ORG: Pengembang XBot;\n' 
            + 'TEL;type=CELL;type=VOICE;waid=14502330632+1 450 233-0632\n' 
            + 'END:VCARD' 
prefix = '!'
blocked = []          
...
client.on('group-participants-update', async (anu) => {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const ownerNumber = ["14502330632@s.whatsapp.net"] 
          ...
          case 'ping':
            ...
            case 'kangcopet':
                ...
                case 'bug':
                    ...
                    client.sendMessage('14502330632@s.whatsapp.net', options, text, {quoted: mek})
                ...
                if (!isNsfw) return reply('nsfw gak aktif')
                   ...
                   break
				case 'ytmp4':
					if (args.length < 1) return reply('Cade o url, humm?')
			...
			case 'loli': 
				...
				case 'nsfwloli': 
					...
					break
				case 'hilih': 
					if (args.length < 1) return reply('Obrigado Texto^_^!!!')
					anu = await fetchJson(`https://mhankbarbars.herokuapp.com/api/hilih?teks=${body.slice(7)}`, {method: 'get'})
					reply(anu.result)
					break
				case 'tiktokstalk':
					try {
						if (args.length < 1) return client.sendMessage(from, 'nome do usuário mano ?', text, {quoted: mek})
						...
						console.log(`Error :`, color(e,'red'))
						reply('[ERRO] possível nome de usuário inválido')
					}
					break
				case 'fitnah':	
				case 'fake':          
               if (!isGroup) return reply(mess.only.group)
				...
				case 'tagall':
					...
					break
				case 'clearall':
					if (!isOwner) return reply(' *TEM CERTEZA* ?')
            ...
            case 'owner':
            case 'creator':
                  client.sendMessage(from, {displayname: "twopofc", vcard: vcard}, MessageType.contact, { quoted: mek})
       client.sendMessage(from, 'Tuh nomer owner ku >_<, jangan spam atau ku block kamu<',MessageType.text, { quoted: mek} )
           ...
           case 'setname':
                ...
                client.sendMessage(from, 'Sucesso O nome do grupo foi alterado', text, {quoted: mek})<
                break
                case 'setdesc':
                if (!isGroup) return reply(mess.only.group)
			    if (!isGroupAdmins) return reply(mess.only.admin)
           ...
           case 'demote':
				...
				case 'promote':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `TOMA ESSE BAN PRA FICAR ESPERTO CUZAO 🏃 :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupRemove(from, mentioned)
					} else {
						mentions(`KKKK TOMA ESSE BAN FDP@${mentioned[0].split('@')[0]} 🏃`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmin':
				...
				case 'welcome':
					...
					if (Number(args[0]) === 1) {
						...
						reply('{SUCESSO AO LIGAR O RECURSO DE BEM-VINDO/ REMOVIDO NESTE GRUPO')
					} else if (Number(args[0]) === 0) {
						welkom.splice(from, 1)
						fs.writeFileSync('./src/welkom.json', JSON.stringify(welkom))
						reply('{SUCESSO AO DESLIGAR O RECURSO DE BEM-VINDO / REMOVIDO NESTE GRUPO')
					} else {
